...
def escape_like(value: str) -> str:
    """Escapa os curingas do LIKE (% e _) e a propria barra de escape.
    Sem isso, um underscore literal na URL (comum em slugs de vaga) casa
    com qualquer caractere e faz duas vagas diferentes parecerem
    duplicatas, descartando a mais nova em silencio."""
...
def find_existing_job(cursor: sqlite3.Cursor, external_id: Optional[str], url: str, dedup_hash: str) -> Optional[sqlite3.Row]:
    ...
    if norm_u:
        like_pattern = f"{escape_like(norm_u)}%"
        cursor.execute(
            "SELECT * FROM jobs WHERE url = ? OR url = ? OR url LIKE ? ESCAPE '\\'",
            (url, norm_u, like_pattern)
        )
        row = cursor.fetchone()
        if row:
            return row
...
def get_jobs(
    ...
    try:
        ...
        if city and city != "todas":
            like_city = f"%{escape_like(city)}%"
            query += " AND (city LIKE ? ESCAPE '\\' OR location LIKE ? ESCAPE '\\')"
            params.append(like_city)
            params.append(like_city)
        ...
        if search:
            ...
            query += " AND (title LIKE ? ESCAPE '\\' OR company LIKE ? ESCAPE '\\' OR description LIKE ? ESCAPE '\\' OR role_matched LIKE ? ESCAPE '\\' OR source LIKE ? ESCAPE '\\')"
